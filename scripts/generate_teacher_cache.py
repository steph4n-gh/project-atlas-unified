#!/usr/bin/env python
"""Generate teacher distillation cache for UCE training.

Runs the unpruned teacher model offline to serialize logits/distributions
and target hidden states to disk, avoiding concurrent VRAM loading during distillation.
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate distillation cache from teacher model.")
    ...
    parser.add_argument("--num-tokens", type=int, default=10000, help="Maximum number of target pairs/tokens to cache")
    parser.add_argument("--batch-size", type=int, default=4, help="Batch size for cache generation")
    parser.add_argument("--dim", type=int, default=16, help="Model/projection dimension")
    parser.add_argument("--out", type=str, required=True, help="Output path for the serialized cache (e.g. teacher_cache.pkl)")
    ...
    if args.synthetic:
        ...
        gm = args.gemma_model
        # Resolve local storage cache if available
        try:
                    ...
                    for line in f:
                        if line.strip():
                            try:
                                ...
                                pass
                print(f"[real] Parsed {len(canned_texts)} JSONL records from {args.corpus_file}")
            else:
